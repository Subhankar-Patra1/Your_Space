    ...
    >
      {images.map((img) => (
        <Rnd
          ...
          onDragStart={() => !isPreview && onSelect(img.id)}
          onResizeStart={() => !isPreview && onSelect(img.id)}
          onClick={(e) => {
             if (isPreview) return;
             e.stopPropagation();
             onSelect(img.id);
          ...
          bounds="parent"
          style={{ 
            pointerEvents: isPreview ? 'none' : 'auto', // Re-enable pointer events for image
          }}
          className={`${!isPreview && img.id === selectedImageId ? 'border-2 border-dashed border-gray-300' : 'border-2 border-transparent hover:border-blue-300'} transition-colors`}
        >
